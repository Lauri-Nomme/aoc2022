...
while (q.length) {
	...
	if (!ms[(steps + 1) % ms.length].has('' + p)) {
	  let key = '' + [(steps + 1) % (W * H), p, phase];
	  let vv = v[key];
	  if (vv == undefined || vv > steps + 1) {
		  v[key] = steps + 1;
		  q.push([p, steps + 1, phase]);
	  }
	}

	for (d of D) {
		let np = add(p, d);
		if (phase == 2 && np[0] == E[0] && np[1] == E[1]) {
			if (steps + 1 < minsteps) {
				console.log('SOL', iter, steps + 1, minsteps, np, phase);
				minsteps = steps + 1;
			}
			continue;
		} else if (phase == 0 && np[0] == E[0] && np[1] == E[1]) {
			console.log('phase 1 reached', steps + 1);
			phase = 1;
		} else if (phase == 1 && np[0] == S[0] && np[1] == S[1]) {
			console.log('phase 2 reached', steps + 1);
			phase = 2;
		} else {
